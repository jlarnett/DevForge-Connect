    ...
    fetch("http://127.0.0.1:8000/genResponse?text=" + message)
        .then(response => {
            if (!response.ok) {
                ...
                var encodedMsg = "<div class='border border-primary rounded-4 m-1'><p><strong>" + "    Chat Bot" + "</strong> <span class='messageTime'>" + time + "</span><hr></p><p>    " + "    I could not process the message. Please try again." + "</p></div>";
        ...
        .then(data => {
            ...
            var encodedMsg = "<div class='border border-primary rounded-4 m-1'><p><strong>" + "    Chat Bot" + "</strong> <span class='messageTime'>" + time + "</span><hr></p><p>    " + data + "</p></div>";
            var element = document.createElement("div");
            element.innerHTML = encodedMsg;
            document.getElementById("messages").appendChild(element);
        ...
        .catch(error => {
            ...
            var encodedMsg = "<div class='border border-primary rounded-4 m-1'><p><strong>" + "    Chat Bot" + "</strong> <span class='messageTime'>" + time + "</span><hr></p><p>    " + "    I could not process the message due to "+ error +". Please try again." + "</p></div>";
            var element = document.createElement("div");
            element.innerHTML = encodedMsg;
            document.getElementById("messages").appendChild(element);
